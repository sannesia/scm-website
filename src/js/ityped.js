  ...
  forEach(
    ...
    function (word, index, arr) {
      ...
      if (element._props.backSpeed < element._props.typeSpeed) {
        time -=
          (element._props.typeSpeed - element._props.backSpeed) * word.length;
      } else if (element._props.backSpeed > element._props.typeSpeed) {
        time +=
          (element._props.backSpeed - element._props.typeSpeed) * word.length;
      }
      let done = this.async();
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (index + 1 === lengthWords) {
      if (!element._props.loop) {
        // when the last word
        if (
          element._props.onFinished !== undefined &&
          typeof element._props.onFinished === 'function'
        ) {
